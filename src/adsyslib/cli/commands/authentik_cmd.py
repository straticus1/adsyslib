...
def get_client() -> AuthentikClient:
    ...
    if not base_url or not token:
        console.print(
            "[bold red]Error:[/bold red] Set AUTHENTIK_URL and AUTHENTIK_TOKEN environment variables."
        )
        raise typer.Exit(1)
...
def create_user(
    ...
    try:
        ...
        console.print(
            f"[bold green]Created user:[/bold green] {user.get('username')} (ID: {user.get('pk')})"
        )
...
def create_group(
    ...
    try:
        ...
        console.print(
            f"[bold green]Created group:[/bold green] {group.get('name')} (UUID: {group.get('pk')})"
        )
    except Exception as e:
        console.print(f"[bold red]Failed:[/bold red] {e}")
        raise typer.Exit(1) from e
...
def create_app(
    ...
    try:
        ...
        console.print(
            f"[bold green]Created application:[/bold green] {app_obj.get('name')} ({app_obj.get('slug')})"
        )
    except Exception as e:
        console.print(f"[bold red]Failed:[/bold red] {e}")
        raise typer.Exit(1) from e
...
def oauth_bulk_create(
    ...
    output_json: Optional[str] = typer.Option(
        None, "--output-json", help="Output JSON file with all secrets"
    ),
) -> None:
...
def oauth_list(
    ...
    try:
        ...
        for p in providers:
            table.add_row(
                p["name"], p["client_id"], p["client_type"], "\n".join(p.get("redirect_uris", []))
            )
